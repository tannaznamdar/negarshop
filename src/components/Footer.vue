<script setup>
import { Splide, SplideSlide } from '@splidejs/vue-splide';
import logo from '../assets/images/logo.png'
</script>

<template>
    <div class="wrapper">

        <!-- footer-lg -->
        <footer class="site-footer responsive-footer-lg">
            <div class="shape-top">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                    <path class="shape-fill" opacity="0.33"
                        d="M473,67.3c-203.9,88.3-263.1-34-320.3,0C66,119.1,0,59.7,0,59.7V0h1000v59.7 c0,0-62.1,26.1-94.9,29.3c-32.8,3.3-62.8-12.3-75.8-22.1C806,49.6,745.3,8.7,694.9,4.7S492.4,59,473,67.3z">
                    </path>
                    <path class="shape-fill" opacity="0.66"
                        d="M734,67.3c-45.5,0-77.2-23.2-129.1-39.1c-28.6-8.7-150.3-10.1-254,39.1 s-91.7-34.4-149.2,0C115.7,118.3,0,39.8,0,39.8V0h1000v36.5c0,0-28.2-18.5-92.1-18.5C810.2,18.1,775.7,67.3,734,67.3z">
                    </path>
                    <path class="shape-fill"
                        d="M766.1,28.9c-200-57.5-266,65.5-395.1,19.5C242,1.8,242,5.4,184.8,20.6C128,35.8,132.3,44.9,89.9,52.5C28.6,63.7,0,0,0,0 h1000c0,0-9.9,40.9-83.6,48.1S829.6,47,766.1,28.9z">
                    </path>
                </svg>
            </div>

            <div class="container">
                <section class="footer-item-box">
                    <div class="row flex-nowrap flex-lg-wrap">
                        <div class="col-auto col-lg d-flex justify-content-between">
                            <div class="d-flex flex-row">
                                <div class="icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40"
                                        viewBox="0 0 512 512" width="40">
                                        <g>
                                            <g>
                                                <path
                                                    d="m502 340.354h-.303v-58.216c-.004-1.814-.366-2.854-.723-3.74l-36.249-95.892c-1.47-3.89-5.194-6.464-9.354-6.464h-59.152v-62.676c0-15.024-12.215-27.246-27.229-27.246h-176.192c3.212 6.331 5.818 13.023 7.742 20h168.45c3.986 0 7.229 3.251 7.229 7.246v226.987h-285.919v-109.155c-6.909-1.099-13.597-2.86-20-5.217v114.373h-.303c-5.523 0-10 4.477-10 10v61.51c0 5.523 4.477 10 10 10h46.407c-.73 3.476-1.114 7.079-1.114 10.77 0 28.725 23.254 52.095 51.838 52.095 28.583 0 51.838-23.37 51.838-52.095 0-3.691-.384-7.294-1.114-10.77h147.24c-.73 3.476-1.114 7.079-1.114 10.77 0 28.725 23.255 52.095 51.839 52.095s51.838-23.37 51.838-52.095c0-3.691-.384-7.294-1.114-10.77h35.459c5.522 0 10-4.477 10-10v-61.51c0-5.522-4.478-10-10-10zm-105.779-144.311h52.24l28.766 76.095h-81.006zm0 96.095h85.477v48.211h-85.477zm-229.092 172.591c-17.556 0-31.838-14.397-31.838-32.095 0-17.697 14.283-32.095 31.838-32.095s31.838 14.397 31.838 32.095-14.282 32.095-31.838 32.095zm248.688 0c-17.556 0-31.839-14.397-31.839-32.095 0-17.697 14.283-32.095 31.839-32.095s31.838 14.397 31.838 32.095-14.282 32.095-31.838 32.095zm76.183-62.865h-34.24c-.045 0-.09.001-.135.002-9.442-12.923-24.666-21.326-41.807-21.326-17.14 0-32.364 8.403-41.806 21.325h-164.933c-.048 0-.095.001-.143.002-9.443-12.923-24.667-21.326-41.807-21.326s-32.363 8.403-41.806 21.325h-45.325v-41.51h412.002z"
                                                    fill="#020288"></path>
                                            </g>
                                            <g>
                                                <ellipse cx="107.207" cy="134.9" fill="#e2c4ff" rx="107.207"
                                                    ry="107.629">
                                                </ellipse>
                                                <path
                                                    d="m125.859 149.581h-18.651c-5.523 0-10-4.477-10-10v-31.208c0-5.523 4.477-10 10-10s10 4.477 10 10v21.208h8.651c5.523 0 10 4.477 10 10 0 5.524-4.477 10-10 10z"
                                                    fill="#020288"></path>
                                            </g>
                                        </g>
                                    </svg>
                                </div>
                                <div class="content d-flex flex-column">
                                    <span class="title-13 title"> تحویل اکسپرس </span>
                                    <span class="subtitle"> در کمترین زمان </span>
                                </div>
                            </div>

                            <div class="d-flex flex-row">
                                <div class="icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40"
                                        viewBox="0 0 512 512" width="40">
                                        <g>
                                            <g>
                                                <path
                                                    d="m292.28 2.536c-66.416 0-125.949 29.445-166.237 75.982 5.624 2.843 10.9 6.619 15.599 11.331l28.33 28.406c29.435-34.59 73.306-56.535 122.308-56.535 88.627 0 160.473 71.785 160.473 160.336 0 49.151-22.135 93.136-56.988 122.548l22.757 22.818c6.75 6.768 11.573 14.741 14.472 23.231 48.277-40.268 78.994-100.848 78.994-168.597-.001-121.237-98.367-219.52-219.708-219.52z"
                                                    fill="#b2f0fb"></path>
                                            </g>
                                            <g fill="#020288">
                                                <path
                                                    d="m425.602 360.361-32.672-32.76c-12.919-12.953-30.086-20.087-48.34-20.087s-35.422 7.134-48.342 20.089l-29.512 29.609-114.89-115.153 29.549-29.646c26.634-26.706 26.634-70.159 0-96.864l-32.672-32.76c-13.35-13.386-31.09-20.758-49.954-20.758s-36.604 7.372-49.954 20.758c-.274.275-.532.565-.772.87l-15.997 20.289c-15.27 19.367-25.587 42.398-29.837 66.604-4.286 24.413-2.311 49.581 5.71 72.784 5.812 16.812 13.122 29.71 21.726 38.338l197.771 198.251c6.821 6.84 16.396 12.92 28.46 18.072 18.002 7.688 37.12 11.456 56.211 11.455 31.786-.001 63.489-10.449 89.775-30.642l22.751-17.479c.348-.267.678-.558.988-.869 27.525-27.598 27.525-72.503.001-100.101zm-13.719 85.528-22.205 17.059c-36.35 27.926-84.611 34.307-125.948 16.655-9.696-4.141-17.149-8.785-22.154-13.803l-197.771-198.251c-6.452-6.47-12.167-16.815-16.983-30.749-14.134-40.886-6.31-85.921 20.93-120.47l15.62-19.811c9.523-9.347 22.072-14.488 35.397-14.488 13.511 0 26.222 5.285 35.792 14.881l32.672 32.76c18.867 18.918 18.867 49.7-.002 68.62l-36.591 36.711c-3.891 3.904-3.89 10.221.003 14.123l129.016 129.311c1.876 1.881 4.425 2.938 7.081 2.937 2.657-.001 5.205-1.059 7.08-2.94l36.589-36.709c9.139-9.164 21.277-14.21 34.179-14.21s25.04 5.046 34.179 14.21l32.672 32.76c19.61 19.661 19.758 51.559.444 71.404z">
                                                </path>
                                                <g>
                                                    <path
                                                        d="m361.003 221.7h-1.121c.121-21.045.184-43.537-.076-46.664-.586-7.056-4.823-12.807-10.796-14.65-5.923-1.828-12.211.471-16.822 6.152-4.979 6.136-27.961 47.742-34.886 60.347-1.702 3.098-1.642 6.864.158 9.907 1.8 3.042 5.072 4.908 8.606 4.908h33.681c-.041 5.609-.084 10.986-.125 15.844-.047 5.523 4.391 10.038 9.914 10.085.029.001.059.001.088.001 5.482 0 9.951-4.421 9.997-9.914.026-2.999.074-8.807.128-16.016h1.253c5.523 0 10-4.477 10-10 .001-5.523-4.477-10-9.999-10zm-21.125 0h-16.853c6.251-11.227 12.345-21.998 16.965-29.943-.004 8.206-.048 18.878-.112 29.943z">
                                                    </path>
                                                    <path
                                                        d="m282.45 250.419c-8.794.111-18.124.184-25.861.199 4.857-6.523 11.614-15.793 20.75-28.902 5.593-8.025 9.24-15.609 10.84-22.541.375-1.584.688-4.6.689-5.647.018-17.91-14.528-32.48-32.385-32.48-15.444 0-28.809 11.02-31.779 26.204-1.06 5.42 2.474 10.673 7.895 11.733 5.419 1.059 10.673-2.475 11.733-7.895 1.138-5.819 6.248-10.042 12.151-10.042 6.649 0 12.09 5.307 12.373 11.938l-.272 2.136c-1.091 4.338-3.664 9.436-7.654 15.16-13.857 19.883-22.126 30.756-26.567 36.598-5.489 7.217-7.994 10.511-6.29 16.041 1.001 3.249 3.461 5.744 6.749 6.844 1.337.448 2.619.877 19.628.877 6.676 0 15.777-.066 28.251-.224 5.522-.069 9.943-4.603 9.873-10.125-.068-5.523-4.619-9.93-10.124-9.874z">
                                                    </path>
                                                </g>
                                            </g>
                                        </g>
                                    </svg>
                                </div>
                                <div class="content d-flex flex-column">
                                    <span class="title-13 title"> پشتیبانی ۲۴ ساعته </span>
                                    <span class="subtitle"> پشتیبانی حرفه ای </span>
                                </div>
                            </div>

                            <div class="d-flex flex-row">
                                <div class="icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40"
                                        viewBox="0 0 512 512" width="40">
                                        <g>
                                            <g>
                                                <path d="m182.8 245.634h146.401v175.733h-146.401z" fill="#b2f0fb">
                                                </path>
                                            </g>
                                            <g fill="#020288">
                                                <path
                                                    d="m249.334 472.084h13.333c5.523 0 10-4.478 10-10 0-5.523-4.477-10-10-10h-13.333c-5.523 0-10 4.477-10 10 0 5.522 4.477 10 10 10z">
                                                </path>
                                                <path
                                                    d="m414.606 147.622v-99.52c0-26.523-21.661-48.102-48.285-48.102h-220.642c-26.624 0-48.284 21.579-48.284 48.103v99.516c-.007.429-.007.857 0 1.284v314.904c0 26.574 21.619 48.193 48.193 48.193h220.825c26.574 0 48.193-21.619 48.193-48.193v-314.908c.006-.424.007-.85 0-1.277zm-20 1.097c-.111 4.863-2.034 9.425-5.466 12.922-3.701 3.773-8.675 5.851-14.005 5.851-10.909 0-19.784-8.875-19.784-19.784 0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784-5.329 0-10.303-2.077-14.004-5.85-3.431-3.497-5.354-8.058-5.466-12.919v-64.178h277.211v64.174zm-277.211 33.701c5.863 3.311 12.52 5.071 19.47 5.071 11.84 0 22.49-5.2 29.784-13.436 7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436 6.95 0 13.607-1.761 19.471-5.071v229.748h-277.213zm28.284-162.42h220.643c15.596 0 28.284 12.606 28.284 28.103v16.442h-277.211v-16.442c0-15.497 12.688-28.103 28.284-28.103zm248.927 443.807c0 15.546-12.648 28.193-28.193 28.193h-220.826c-15.545 0-28.193-12.647-28.193-28.193v-31.639h277.211v31.639z">
                                                </path>
                                            </g>
                                        </g>
                                    </svg>
                                </div>
                                <div class="content d-flex flex-column">
                                    <span class="title-13 title">پرداخت در محل </span>
                                    <span class="subtitle"> پرداخت هنگام دریافت </span>
                                </div>
                            </div>

                            <div class="d-flex flex-row">
                                <div class="icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40"
                                        viewBox="0 0 512 512" width="40">
                                        <g>
                                            <g>
                                                <path
                                                    d="m469.871 159.668c1.879-1.876 2.935-4.422 2.935-7.077s-1.056-5.201-2.935-7.077l-53.256-53.161c-3.904-3.896-10.226-3.896-14.13 0l-27.381 27.332c-24.474-16.378-51.956-27.45-81.005-32.619 8.582-9.254 13.835-21.629 13.835-35.207 0-28.595-23.297-51.859-51.934-51.859s-51.934 23.264-51.934 51.859c0 13.577 5.252 25.953 13.835 35.207-29.049 5.169-56.531 16.241-81.005 32.619l-27.381-27.332c-3.904-3.896-10.226-3.896-14.13 0l-53.256 53.161c-1.879 1.876-2.935 4.422-2.935 7.077s1.056 5.201 2.935 7.077l29.173 29.122c-19.437 32.673-29.807 70.076-29.807 109.069 0 57.202 22.314 110.98 62.833 151.427 40.514 40.442 94.379 62.714 151.672 62.714s111.158-22.272 151.671-62.714c40.519-40.446 62.833-94.225 62.833-151.427 0-38.994-10.37-76.396-29.807-109.07zm-60.321-46.108 39.101 39.031-19.215 19.181c-6.53-8.912-13.795-17.383-21.765-25.339-5.221-5.211-10.664-10.121-16.307-14.72zm-153.55-93.56c17.608 0 31.934 14.292 31.934 31.859 0 14.084-9.209 26.063-21.934 30.259v-23.11c0-5.522-4.477-10-10-10s-10 4.478-10 10v23.11c-12.726-4.196-21.934-16.175-21.934-30.259 0-17.567 14.326-31.859 31.934-31.859zm-192.651 132.591 39.101-39.031 18.186 18.153c-5.643 4.599-11.086 9.508-16.307 14.72-7.97 7.956-15.235 16.428-21.765 25.339zm192.651 339.409c-107.251 0-194.505-87.091-194.505-194.141s87.254-194.14 194.505-194.14 194.505 87.091 194.505 194.141-87.255 194.14-194.505 194.14z"
                                                    fill="#020288"></path>
                                            </g>
                                            <g>
                                                <g>
                                                    <ellipse cx="256" cy="297.859" fill="#e2c4ff" rx="152.994"
                                                        ry="152.721">
                                                    </ellipse>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="m256 195.824c-5.523 0-10-4.478-10-10v-3.411c0-5.523 4.477-10 10-10s10 4.477 10 10v3.411c0 5.523-4.477 10-10 10z"
                                                            fill="#020288"></path>
                                                    </g>
                                                    <g>
                                                        <path
                                                            d="m256 423.306c-5.523 0-10-4.478-10-10v-3.411c0-5.522 4.477-10 10-10s10 4.478 10 10v3.411c0 5.522-4.477 10-10 10z"
                                                            fill="#020288"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="m371.446 307.859h-3.412c-5.523 0-10-4.478-10-10s4.477-10 10-10h3.412c5.523 0 10 4.478 10 10 0 5.523-4.477 10-10 10z"
                                                            fill="#020288"></path>
                                                    </g>
                                                    <g>
                                                        <path
                                                            d="m143.965 307.859h-3.412c-5.523 0-10-4.478-10-10s4.477-10 10-10h3.412c5.523 0 10 4.478 10 10 0 5.523-4.477 10-10 10z"
                                                            fill="#020288"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="m335.221 228.639c-2.559 0-5.119-.977-7.071-2.929-3.905-3.905-3.905-10.237 0-14.143l2.412-2.412c3.905-3.904 10.237-3.904 14.143 0 3.905 3.905 3.905 10.237 0 14.143l-2.412 2.412c-1.954 1.952-4.513 2.929-7.072 2.929z"
                                                            fill="#020288"></path>
                                                    </g>
                                                    <g>
                                                        <path
                                                            d="m174.367 389.492c-2.559 0-5.119-.977-7.071-2.929-3.905-3.905-3.905-10.237 0-14.143l2.412-2.412c3.905-3.904 10.237-3.904 14.143 0 3.905 3.905 3.905 10.237 0 14.143l-2.412 2.412c-1.953 1.953-4.513 2.929-7.072 2.929z"
                                                            fill="#020288"></path>
                                                    </g>
                                                </g>
                                                <g>
                                                    <g>
                                                        <path
                                                            d="m337.633 389.492c-2.559 0-5.119-.977-7.071-2.929l-2.412-2.412c-3.905-3.905-3.905-10.237 0-14.143 3.905-3.904 10.237-3.904 14.143 0l2.412 2.412c3.905 3.905 3.905 10.237 0 14.143-1.954 1.953-4.513 2.929-7.072 2.929z"
                                                            fill="#020288"></path>
                                                    </g>
                                                    <g>
                                                        <path
                                                            d="m176.779 228.639c-2.559 0-5.119-.977-7.071-2.929l-2.412-2.412c-3.905-3.905-3.905-10.237 0-14.143 3.905-3.904 10.237-3.904 14.143 0l2.412 2.412c3.905 3.905 3.905 10.237 0 14.143-1.953 1.952-4.513 2.929-7.072 2.929z"
                                                            fill="#020288"></path>
                                                    </g>
                                                </g>
                                                <path
                                                    d="m315.809 286.361h-25.278c-3.337-11.001-12.031-19.695-23.033-23.032v-31.308c0-5.523-4.477-10-10-10s-10 4.477-10 10v31.308c-14.168 4.297-24.511 17.48-24.511 33.032 0 19.029 15.482 34.511 34.511 34.511 15.553 0 28.736-10.342 33.033-24.511h25.278c5.523 0 10-4.478 10-10s-4.477-10-10-10zm-58.311 24.511c-8.001 0-14.511-6.51-14.511-14.511s6.509-14.51 14.511-14.51c8.001 0 14.511 6.509 14.511 14.51-.001 8.001-6.51 14.511-14.511 14.511z"
                                                    fill="#020288"></path>
                                            </g>
                                        </g>
                                    </svg>
                                </div>
                                <div class="content d-flex flex-column">
                                    <span class="title-13 title"> ۷ روز ضمانت </span>
                                    <span class="subtitle"> مهلت بازگشت وجه </span>
                                </div>
                            </div>

                            <div class="d-flex flex-row">
                                <div class="icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40"
                                        viewBox="0 0 512 512" width="40">
                                        <g>
                                            <g fill="#020288">
                                                <path
                                                    d="m256 82.637c-95.593 0-173.363 77.77-173.363 173.363s77.77 173.363 173.363 173.363 173.363-77.77 173.363-173.363-77.77-173.363-173.363-173.363zm0 326.726c-84.565 0-153.363-68.798-153.363-153.363s68.798-153.363 153.363-153.363 153.363 68.798 153.363 153.363-68.799 153.363-153.363 153.363z">
                                                </path>
                                                <path
                                                    d="m498.337 290.378c6.404-9.956 13.663-21.24 13.663-34.378s-7.259-24.422-13.663-34.378c-4.287-6.664-8.336-12.958-9.635-18.665-1.386-6.087-.459-13.74.522-21.841 1.467-12.119 2.985-24.65-2.533-36.07-5.615-11.621-16.971-18.584-26.991-24.728-6.866-4.209-13.351-8.186-17.123-12.904-3.812-4.767-6.274-11.998-8.88-19.653-3.772-11.079-8.046-23.635-18.077-31.633-9.979-7.956-23.151-9.336-34.774-10.554-8.088-.847-15.728-1.647-21.29-4.327-5.372-2.588-10.659-7.975-16.256-13.678-8.265-8.422-17.633-17.968-30.34-20.853-12.233-2.776-24.047 1.462-35.472 5.563-7.769 2.787-15.108 5.421-21.488 5.421s-13.719-2.634-21.488-5.422c-11.424-4.1-23.237-8.339-35.472-5.563-12.707 2.884-22.075 12.43-30.34 20.853-5.597 5.703-10.883 11.09-16.255 13.678-5.562 2.679-13.201 3.479-21.29 4.327-11.623 1.218-24.796 2.598-34.774 10.554-10.031 7.998-14.306 20.554-18.077 31.632-2.606 7.656-5.068 14.887-8.88 19.654-3.772 4.718-10.258 8.694-17.124 12.904-10.019 6.144-21.375 13.107-26.99 24.728-5.518 11.421-4.001 23.952-2.534 36.071.981 8.102 1.908 15.754.522 21.841-1.299 5.707-5.348 12.001-9.635 18.665-6.404 9.956-13.663 21.24-13.663 34.378s7.259 24.422 13.663 34.377c4.287 6.665 8.336 12.959 9.635 18.666 1.386 6.087.459 13.739-.522 21.841-1.467 12.119-2.985 24.65 2.533 36.07 5.615 11.621 16.971 18.585 26.991 24.728 6.866 4.21 13.351 8.186 17.123 12.904 3.812 4.767 6.274 11.998 8.88 19.653 3.771 11.079 8.046 23.635 18.077 31.633 9.979 7.956 23.152 9.336 34.774 10.554 8.088.847 15.728 1.647 21.29 4.327 5.372 2.588 10.659 7.975 16.256 13.678 8.265 8.422 17.633 17.968 30.34 20.853 12.236 2.777 24.048-1.461 35.472-5.563 7.77-2.788 15.108-5.422 21.488-5.422s13.719 2.634 21.488 5.422c8.874 3.185 17.98 6.453 27.338 6.453 2.69 0 5.402-.27 8.134-.89 12.707-2.884 22.075-12.43 30.34-20.852 5.597-5.704 10.883-11.09 16.255-13.678 5.562-2.68 13.202-3.48 21.29-4.327 11.623-1.218 24.795-2.598 34.774-10.554 10.031-7.998 14.305-20.554 18.077-31.632 2.606-7.656 5.068-14.887 8.88-19.654 3.772-4.718 10.258-8.694 17.124-12.904 10.019-6.144 21.375-13.107 26.99-24.728 5.518-11.421 4.001-23.952 2.534-36.071-.981-8.102-1.908-15.754-.522-21.841 1.298-5.707 5.348-12.001 9.635-18.665zm-16.82-10.821c-5.033 7.824-10.237 15.914-12.316 25.046-2.158 9.478-.976 19.243.168 28.685 1.172 9.682 2.279 18.827-.687 24.966-3.063 6.338-11.016 11.215-19.437 16.378-8.029 4.923-16.332 10.014-22.289 17.463-5.998 7.5-9.147 16.751-12.193 25.699-3.167 9.304-6.16 18.092-11.613 22.44-5.407 4.312-14.628 5.277-24.389 6.3-9.434.988-19.189 2.01-27.886 6.2-8.52 4.104-15.297 11.01-21.851 17.688-6.951 7.083-13.517 13.773-20.492 15.357-6.544 1.483-15.164-1.608-24.29-4.883-9.038-3.244-18.384-6.598-28.244-6.598s-19.206 3.354-28.244 6.598c-9.126 3.275-17.746 6.368-24.29 4.883-6.975-1.584-13.541-8.274-20.492-15.357-6.554-6.678-13.331-13.584-21.851-17.688-8.697-4.19-18.452-5.211-27.886-6.2-9.762-1.022-18.982-1.989-24.389-6.3-5.453-4.348-8.445-13.136-11.612-22.44-3.046-8.947-6.196-18.199-12.193-25.699-5.957-7.449-14.259-12.539-22.289-17.463-8.421-5.163-16.375-10.039-19.437-16.378-2.966-6.139-1.859-15.284-.687-24.966 1.144-9.442 2.326-19.206.168-28.685-2.079-9.132-7.283-17.223-12.316-25.047-5.388-8.379-10.48-16.296-10.48-23.556 0-7.261 5.092-15.177 10.483-23.558 5.033-7.824 10.237-15.914 12.316-25.046 2.158-9.479.976-19.243-.168-28.685-1.172-9.682-2.279-18.827.687-24.966 3.063-6.338 11.016-11.215 19.437-16.378 8.029-4.923 16.332-10.014 22.289-17.463 5.997-7.499 9.147-16.752 12.193-25.699 3.168-9.304 6.16-18.092 11.613-22.44 5.408-4.312 14.628-5.277 24.39-6.3 9.434-.988 19.188-2.01 27.886-6.199 8.52-4.104 15.296-11.01 21.85-17.688 6.951-7.083 13.517-13.773 20.492-15.357 6.542-1.484 15.164 1.608 24.29 4.883 9.036 3.242 18.382 6.596 28.242 6.596s19.206-3.354 28.244-6.598c9.126-3.274 17.744-6.367 24.29-4.883 6.975 1.583 13.541 8.274 20.492 15.357 6.554 6.678 13.331 13.584 21.851 17.688 8.697 4.189 18.452 5.211 27.886 6.2 9.762 1.023 18.982 1.989 24.39 6.3 5.453 4.348 8.445 13.136 11.612 22.441 3.046 8.947 6.196 18.199 12.193 25.699 5.957 7.449 14.259 12.54 22.289 17.463 8.421 5.163 16.375 10.039 19.437 16.378 2.966 6.139 1.859 15.284.687 24.966-1.144 9.442-2.326 19.207-.168 28.685 2.079 9.132 7.283 17.222 12.316 25.046 5.389 8.381 10.481 16.297 10.481 23.558 0 7.26-5.092 15.177-10.483 23.557z">
                                                </path>
                                            </g>
                                            <g>
                                                <path
                                                    d="m256 323.636-57.162 20.326 1.668-60.645-36.995-48.082 58.192-17.154 34.297-50.044 34.297 50.044 58.192 17.154-36.995 48.082 1.668 60.645z"
                                                    fill="#e2c4ff"></path>
                                            </g>
                                        </g>
                                    </svg>
                                </div>
                                <div class="content d-flex flex-column">
                                    <span class="title-13 title"> ضمانت اصل‌بودن کالا </span>
                                    <span class="subtitle"> تایید اصالت کالا </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>

                <section class="mb-4">
                    <div class="row">
                        <div class="col-lg-7">
                            <div class="d-flex">
                                <div class="col-lg-4">
                                    <div class="p-10">
                                        <span class="title-14 title "> با نگارشاپ </span>
                                        <div class="list-item">
                                            <ul>
                                                <li>
                                                    <router-link> اتاق خبر نگارشاپ </router-link>
                                                </li>
                                                <li>
                                                    <router-link> فروش در نگارشاپ </router-link>
                                                </li>
                                                <li>
                                                    <router-link>همکاری با سازمان‌ها </router-link>
                                                </li>
                                                <li>
                                                    <router-link> فرصت‌های شغلی </router-link>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-lg-4">
                                    <div class="p-10">
                                        <span class="title-14 title "> خدمات مشتریان </span>
                                        <div class="list-item">
                                            <ul>
                                                <li>
                                                    <router-link> پاسخ به پرسش‌های متداول </router-link>
                                                </li>
                                                <li>
                                                    <router-link> رویه‌های بازگرداندن کالا </router-link>
                                                </li>
                                                <li>
                                                    <router-link> شرایط استفاده </router-link>
                                                </li>
                                                <li>
                                                    <router-link> حریم خصوصی </router-link>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-lg-4">
                                    <div class="p-10">
                                        <span class="title-14 title "> راهنمای خرید از نگارشاپ </span>
                                        <div class="list-item">
                                            <ul>
                                                <li>
                                                    <router-link> نحوه ثبت سفارش </router-link>
                                                </li>
                                                <li>
                                                    <router-link> رویه ارسال سفارش </router-link>
                                                </li>
                                                <li>
                                                    <router-link> شیوه‌های پرداخت </router-link>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="col-lg-2"></div>

                        <div class="col-lg-3">
                            <div class="p-10">
                                <div class="d-flex flex-column">
                                    <span class="title-14 title "> خبرنامه </span>
                                    <span class="title-13 title "> با عضویت در خبرنامه از آخرین اخبار و محصولات سایت
                                        مطلع شوید... </span>
                                </div>
                                <div class="input-form">
                                    <input type="email" placeholder="آدرس ایمیل">
                                    <button class="input-append" type="submit"> عضوم کن </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>

                <section>
                    <div class="row align-items-center mb-3">
                        <div class="col-lg-2">
                            <div class="p-10 text-center">
                                <router-link to="/">
                                    <img class="logo-img" :src="logo" alt="logo">
                                </router-link>
                            </div>
                        </div>

                        <div class="col-lg-8">
                            <span class="title-14 title p-10"> با نگارشاپ </span>
                            <p class="desk p-10"> دیجی‌کالا به عنوان یکی از قدیمی‌ترین فروشگاه های اینترنتی با بیش از یک
                                دهه تجربه، با
                                پایبندی به سه اصل کلیدی، پرداخت در محل، 7 روز ضمانت بازگشت کالا و تضمین اصل‌بودن کالا،
                                موفق شده تا همگام با فروشگاه‌های معتبر جهان، به بزرگ‌ترین فروشگاه اینترنتی ایران تبدیل
                                شود. به محض ورود به دیجی‌کالا با یک سایت پر از کالا رو به رو می‌شوید! هر آنچه که نیاز
                                دارید و به ذهن شما خطور می‌کند در اینجا پیدا خواهید کرد. </p>
                        </div>

                        <div class="col-lg-2 text-center">
                            <Splide class="row" :options="{
                                arrows: false, pagination: false, direction: 'rtl', type: 'loop', perPage: 1, trimSpace: false, perMove: 1, width: 209, autoplay: true, interval: '2000',
                            }" aria-label="My Favorite Images">

                                <SplideSlide class="slider" v-for="symbolOfTrust in symbolOfTrusts">
                                    <img alt={{alt}} :src="symbolOfTrust.thumbnail">
                                </SplideSlide>
                            </Splide>
                        </div>
                    </div>
                </section>

                <section>
                    <p class="title-13 title ">تمامی مطالب، عکس ها و… مطعلق به سایت نگارشاپ می باشد. طراح: پسرک کدنویس
                    </p>
                </section>

                <section>
                    <ul class="fixed-bottom-bar">
                        <li>
                            <button class="bottom-bar" type="button" aria-label="مقایسه" data-toggle="tooltip"
                                data-placement="top" title="" data-original-title="مقایسه">
                                <font-awesome-icon icon="shuffle" />
                            </button>
                        </li>
                        <li>
                            <button class="bottom-bar" type="button">
                                <font-awesome-icon icon="heart" />
                            </button>
                        </li>
                        <li>
                            <button class="bottom-bar" type="button">
                                <font-awesome-icon icon="cart-shopping" />
                            </button>
                        </li>
                        <li>
                            <button class="bottom-bar" type="button">
                                <font-awesome-icon icon="angle-up" />
                            </button>
                        </li>
                    </ul>
                </section>
            </div>
        </footer>

        <!-- footer-md -->
        <footer class="site-footer responsive-footer-tablet">
            <div class="shape-top">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                    <path class="shape-fill" opacity="0.33"
                        d="M473,67.3c-203.9,88.3-263.1-34-320.3,0C66,119.1,0,59.7,0,59.7V0h1000v59.7 c0,0-62.1,26.1-94.9,29.3c-32.8,3.3-62.8-12.3-75.8-22.1C806,49.6,745.3,8.7,694.9,4.7S492.4,59,473,67.3z">
                    </path>
                    <path class="shape-fill" opacity="0.66"
                        d="M734,67.3c-45.5,0-77.2-23.2-129.1-39.1c-28.6-8.7-150.3-10.1-254,39.1 s-91.7-34.4-149.2,0C115.7,118.3,0,39.8,0,39.8V0h1000v36.5c0,0-28.2-18.5-92.1-18.5C810.2,18.1,775.7,67.3,734,67.3z">
                    </path>
                    <path class="shape-fill"
                        d="M766.1,28.9c-200-57.5-266,65.5-395.1,19.5C242,1.8,242,5.4,184.8,20.6C128,35.8,132.3,44.9,89.9,52.5C28.6,63.7,0,0,0,0 h1000c0,0-9.9,40.9-83.6,48.1S829.6,47,766.1,28.9z">
                    </path>
                </svg>
            </div>

            <div class="container">
                <section class="footer-item-box mb-5">
                    <div class="row flex-nowrap flex-lg-wrap mb-4">
                        <div class="col-md-4 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path
                                                d="m502 340.354h-.303v-58.216c-.004-1.814-.366-2.854-.723-3.74l-36.249-95.892c-1.47-3.89-5.194-6.464-9.354-6.464h-59.152v-62.676c0-15.024-12.215-27.246-27.229-27.246h-176.192c3.212 6.331 5.818 13.023 7.742 20h168.45c3.986 0 7.229 3.251 7.229 7.246v226.987h-285.919v-109.155c-6.909-1.099-13.597-2.86-20-5.217v114.373h-.303c-5.523 0-10 4.477-10 10v61.51c0 5.523 4.477 10 10 10h46.407c-.73 3.476-1.114 7.079-1.114 10.77 0 28.725 23.254 52.095 51.838 52.095 28.583 0 51.838-23.37 51.838-52.095 0-3.691-.384-7.294-1.114-10.77h147.24c-.73 3.476-1.114 7.079-1.114 10.77 0 28.725 23.255 52.095 51.839 52.095s51.838-23.37 51.838-52.095c0-3.691-.384-7.294-1.114-10.77h35.459c5.522 0 10-4.477 10-10v-61.51c0-5.522-4.478-10-10-10zm-105.779-144.311h52.24l28.766 76.095h-81.006zm0 96.095h85.477v48.211h-85.477zm-229.092 172.591c-17.556 0-31.838-14.397-31.838-32.095 0-17.697 14.283-32.095 31.838-32.095s31.838 14.397 31.838 32.095-14.282 32.095-31.838 32.095zm248.688 0c-17.556 0-31.839-14.397-31.839-32.095 0-17.697 14.283-32.095 31.839-32.095s31.838 14.397 31.838 32.095-14.282 32.095-31.838 32.095zm76.183-62.865h-34.24c-.045 0-.09.001-.135.002-9.442-12.923-24.666-21.326-41.807-21.326-17.14 0-32.364 8.403-41.806 21.325h-164.933c-.048 0-.095.001-.143.002-9.443-12.923-24.667-21.326-41.807-21.326s-32.363 8.403-41.806 21.325h-45.325v-41.51h412.002z"
                                                fill="#020288"></path>
                                        </g>
                                        <g>
                                            <ellipse cx="107.207" cy="134.9" fill="#e2c4ff" rx="107.207" ry="107.629">
                                            </ellipse>
                                            <path
                                                d="m125.859 149.581h-18.651c-5.523 0-10-4.477-10-10v-31.208c0-5.523 4.477-10 10-10s10 4.477 10 10v21.208h8.651c5.523 0 10 4.477 10 10 0 5.524-4.477 10-10 10z"
                                                fill="#020288"></path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> تحویل اکسپرس </span>
                                <span class="subtitle"> در کمترین زمان </span>
                            </div>
                        </div>

                        <div class="col-md-4 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path
                                                d="m292.28 2.536c-66.416 0-125.949 29.445-166.237 75.982 5.624 2.843 10.9 6.619 15.599 11.331l28.33 28.406c29.435-34.59 73.306-56.535 122.308-56.535 88.627 0 160.473 71.785 160.473 160.336 0 49.151-22.135 93.136-56.988 122.548l22.757 22.818c6.75 6.768 11.573 14.741 14.472 23.231 48.277-40.268 78.994-100.848 78.994-168.597-.001-121.237-98.367-219.52-219.708-219.52z"
                                                fill="#b2f0fb"></path>
                                        </g>
                                        <g fill="#020288">
                                            <path
                                                d="m425.602 360.361-32.672-32.76c-12.919-12.953-30.086-20.087-48.34-20.087s-35.422 7.134-48.342 20.089l-29.512 29.609-114.89-115.153 29.549-29.646c26.634-26.706 26.634-70.159 0-96.864l-32.672-32.76c-13.35-13.386-31.09-20.758-49.954-20.758s-36.604 7.372-49.954 20.758c-.274.275-.532.565-.772.87l-15.997 20.289c-15.27 19.367-25.587 42.398-29.837 66.604-4.286 24.413-2.311 49.581 5.71 72.784 5.812 16.812 13.122 29.71 21.726 38.338l197.771 198.251c6.821 6.84 16.396 12.92 28.46 18.072 18.002 7.688 37.12 11.456 56.211 11.455 31.786-.001 63.489-10.449 89.775-30.642l22.751-17.479c.348-.267.678-.558.988-.869 27.525-27.598 27.525-72.503.001-100.101zm-13.719 85.528-22.205 17.059c-36.35 27.926-84.611 34.307-125.948 16.655-9.696-4.141-17.149-8.785-22.154-13.803l-197.771-198.251c-6.452-6.47-12.167-16.815-16.983-30.749-14.134-40.886-6.31-85.921 20.93-120.47l15.62-19.811c9.523-9.347 22.072-14.488 35.397-14.488 13.511 0 26.222 5.285 35.792 14.881l32.672 32.76c18.867 18.918 18.867 49.7-.002 68.62l-36.591 36.711c-3.891 3.904-3.89 10.221.003 14.123l129.016 129.311c1.876 1.881 4.425 2.938 7.081 2.937 2.657-.001 5.205-1.059 7.08-2.94l36.589-36.709c9.139-9.164 21.277-14.21 34.179-14.21s25.04 5.046 34.179 14.21l32.672 32.76c19.61 19.661 19.758 51.559.444 71.404z">
                                            </path>
                                            <g>
                                                <path
                                                    d="m361.003 221.7h-1.121c.121-21.045.184-43.537-.076-46.664-.586-7.056-4.823-12.807-10.796-14.65-5.923-1.828-12.211.471-16.822 6.152-4.979 6.136-27.961 47.742-34.886 60.347-1.702 3.098-1.642 6.864.158 9.907 1.8 3.042 5.072 4.908 8.606 4.908h33.681c-.041 5.609-.084 10.986-.125 15.844-.047 5.523 4.391 10.038 9.914 10.085.029.001.059.001.088.001 5.482 0 9.951-4.421 9.997-9.914.026-2.999.074-8.807.128-16.016h1.253c5.523 0 10-4.477 10-10 .001-5.523-4.477-10-9.999-10zm-21.125 0h-16.853c6.251-11.227 12.345-21.998 16.965-29.943-.004 8.206-.048 18.878-.112 29.943z">
                                                </path>
                                                <path
                                                    d="m282.45 250.419c-8.794.111-18.124.184-25.861.199 4.857-6.523 11.614-15.793 20.75-28.902 5.593-8.025 9.24-15.609 10.84-22.541.375-1.584.688-4.6.689-5.647.018-17.91-14.528-32.48-32.385-32.48-15.444 0-28.809 11.02-31.779 26.204-1.06 5.42 2.474 10.673 7.895 11.733 5.419 1.059 10.673-2.475 11.733-7.895 1.138-5.819 6.248-10.042 12.151-10.042 6.649 0 12.09 5.307 12.373 11.938l-.272 2.136c-1.091 4.338-3.664 9.436-7.654 15.16-13.857 19.883-22.126 30.756-26.567 36.598-5.489 7.217-7.994 10.511-6.29 16.041 1.001 3.249 3.461 5.744 6.749 6.844 1.337.448 2.619.877 19.628.877 6.676 0 15.777-.066 28.251-.224 5.522-.069 9.943-4.603 9.873-10.125-.068-5.523-4.619-9.93-10.124-9.874z">
                                                </path>
                                            </g>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> پشتیبانی ۲۴ ساعته </span>
                                <span class="subtitle"> پشتیبانی حرفه ای </span>
                            </div>
                        </div>

                        <div class="col-md-4 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path d="m182.8 245.634h146.401v175.733h-146.401z" fill="#b2f0fb">
                                            </path>
                                        </g>
                                        <g fill="#020288">
                                            <path
                                                d="m249.334 472.084h13.333c5.523 0 10-4.478 10-10 0-5.523-4.477-10-10-10h-13.333c-5.523 0-10 4.477-10 10 0 5.522 4.477 10 10 10z">
                                            </path>
                                            <path
                                                d="m414.606 147.622v-99.52c0-26.523-21.661-48.102-48.285-48.102h-220.642c-26.624 0-48.284 21.579-48.284 48.103v99.516c-.007.429-.007.857 0 1.284v314.904c0 26.574 21.619 48.193 48.193 48.193h220.825c26.574 0 48.193-21.619 48.193-48.193v-314.908c.006-.424.007-.85 0-1.277zm-20 1.097c-.111 4.863-2.034 9.425-5.466 12.922-3.701 3.773-8.675 5.851-14.005 5.851-10.909 0-19.784-8.875-19.784-19.784 0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784-5.329 0-10.303-2.077-14.004-5.85-3.431-3.497-5.354-8.058-5.466-12.919v-64.178h277.211v64.174zm-277.211 33.701c5.863 3.311 12.52 5.071 19.47 5.071 11.84 0 22.49-5.2 29.784-13.436 7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436 6.95 0 13.607-1.761 19.471-5.071v229.748h-277.213zm28.284-162.42h220.643c15.596 0 28.284 12.606 28.284 28.103v16.442h-277.211v-16.442c0-15.497 12.688-28.103 28.284-28.103zm248.927 443.807c0 15.546-12.648 28.193-28.193 28.193h-220.826c-15.545 0-28.193-12.647-28.193-28.193v-31.639h277.211v31.639z">
                                            </path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title">پرداخت در محل </span>
                                <span class="subtitle"> پرداخت هنگام دریافت </span>
                            </div>
                        </div>
                    </div>

                    <div class="row flex-nowrap flex-lg-wrap">
                        <div class="col-md-4 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path
                                                d="m469.871 159.668c1.879-1.876 2.935-4.422 2.935-7.077s-1.056-5.201-2.935-7.077l-53.256-53.161c-3.904-3.896-10.226-3.896-14.13 0l-27.381 27.332c-24.474-16.378-51.956-27.45-81.005-32.619 8.582-9.254 13.835-21.629 13.835-35.207 0-28.595-23.297-51.859-51.934-51.859s-51.934 23.264-51.934 51.859c0 13.577 5.252 25.953 13.835 35.207-29.049 5.169-56.531 16.241-81.005 32.619l-27.381-27.332c-3.904-3.896-10.226-3.896-14.13 0l-53.256 53.161c-1.879 1.876-2.935 4.422-2.935 7.077s1.056 5.201 2.935 7.077l29.173 29.122c-19.437 32.673-29.807 70.076-29.807 109.069 0 57.202 22.314 110.98 62.833 151.427 40.514 40.442 94.379 62.714 151.672 62.714s111.158-22.272 151.671-62.714c40.519-40.446 62.833-94.225 62.833-151.427 0-38.994-10.37-76.396-29.807-109.07zm-60.321-46.108 39.101 39.031-19.215 19.181c-6.53-8.912-13.795-17.383-21.765-25.339-5.221-5.211-10.664-10.121-16.307-14.72zm-153.55-93.56c17.608 0 31.934 14.292 31.934 31.859 0 14.084-9.209 26.063-21.934 30.259v-23.11c0-5.522-4.477-10-10-10s-10 4.478-10 10v23.11c-12.726-4.196-21.934-16.175-21.934-30.259 0-17.567 14.326-31.859 31.934-31.859zm-192.651 132.591 39.101-39.031 18.186 18.153c-5.643 4.599-11.086 9.508-16.307 14.72-7.97 7.956-15.235 16.428-21.765 25.339zm192.651 339.409c-107.251 0-194.505-87.091-194.505-194.141s87.254-194.14 194.505-194.14 194.505 87.091 194.505 194.141-87.255 194.14-194.505 194.14z"
                                                fill="#020288"></path>
                                        </g>
                                        <g>
                                            <g>
                                                <ellipse cx="256" cy="297.859" fill="#e2c4ff" rx="152.994" ry="152.721">
                                                </ellipse>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m256 195.824c-5.523 0-10-4.478-10-10v-3.411c0-5.523 4.477-10 10-10s10 4.477 10 10v3.411c0 5.523-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m256 423.306c-5.523 0-10-4.478-10-10v-3.411c0-5.522 4.477-10 10-10s10 4.478 10 10v3.411c0 5.522-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m371.446 307.859h-3.412c-5.523 0-10-4.478-10-10s4.477-10 10-10h3.412c5.523 0 10 4.478 10 10 0 5.523-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m143.965 307.859h-3.412c-5.523 0-10-4.478-10-10s4.477-10 10-10h3.412c5.523 0 10 4.478 10 10 0 5.523-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m335.221 228.639c-2.559 0-5.119-.977-7.071-2.929-3.905-3.905-3.905-10.237 0-14.143l2.412-2.412c3.905-3.904 10.237-3.904 14.143 0 3.905 3.905 3.905 10.237 0 14.143l-2.412 2.412c-1.954 1.952-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m174.367 389.492c-2.559 0-5.119-.977-7.071-2.929-3.905-3.905-3.905-10.237 0-14.143l2.412-2.412c3.905-3.904 10.237-3.904 14.143 0 3.905 3.905 3.905 10.237 0 14.143l-2.412 2.412c-1.953 1.953-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m337.633 389.492c-2.559 0-5.119-.977-7.071-2.929l-2.412-2.412c-3.905-3.905-3.905-10.237 0-14.143 3.905-3.904 10.237-3.904 14.143 0l2.412 2.412c3.905 3.905 3.905 10.237 0 14.143-1.954 1.953-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m176.779 228.639c-2.559 0-5.119-.977-7.071-2.929l-2.412-2.412c-3.905-3.905-3.905-10.237 0-14.143 3.905-3.904 10.237-3.904 14.143 0l2.412 2.412c3.905 3.905 3.905 10.237 0 14.143-1.953 1.952-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <path
                                                d="m315.809 286.361h-25.278c-3.337-11.001-12.031-19.695-23.033-23.032v-31.308c0-5.523-4.477-10-10-10s-10 4.477-10 10v31.308c-14.168 4.297-24.511 17.48-24.511 33.032 0 19.029 15.482 34.511 34.511 34.511 15.553 0 28.736-10.342 33.033-24.511h25.278c5.523 0 10-4.478 10-10s-4.477-10-10-10zm-58.311 24.511c-8.001 0-14.511-6.51-14.511-14.511s6.509-14.51 14.511-14.51c8.001 0 14.511 6.509 14.511 14.51-.001 8.001-6.51 14.511-14.511 14.511z"
                                                fill="#020288"></path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> ۷ روز ضمانت </span>
                                <span class="subtitle"> مهلت بازگشت وجه </span>
                            </div>
                        </div>

                        <div class="col-md-4 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g fill="#020288">
                                            <path
                                                d="m256 82.637c-95.593 0-173.363 77.77-173.363 173.363s77.77 173.363 173.363 173.363 173.363-77.77 173.363-173.363-77.77-173.363-173.363-173.363zm0 326.726c-84.565 0-153.363-68.798-153.363-153.363s68.798-153.363 153.363-153.363 153.363 68.798 153.363 153.363-68.799 153.363-153.363 153.363z">
                                            </path>
                                            <path
                                                d="m498.337 290.378c6.404-9.956 13.663-21.24 13.663-34.378s-7.259-24.422-13.663-34.378c-4.287-6.664-8.336-12.958-9.635-18.665-1.386-6.087-.459-13.74.522-21.841 1.467-12.119 2.985-24.65-2.533-36.07-5.615-11.621-16.971-18.584-26.991-24.728-6.866-4.209-13.351-8.186-17.123-12.904-3.812-4.767-6.274-11.998-8.88-19.653-3.772-11.079-8.046-23.635-18.077-31.633-9.979-7.956-23.151-9.336-34.774-10.554-8.088-.847-15.728-1.647-21.29-4.327-5.372-2.588-10.659-7.975-16.256-13.678-8.265-8.422-17.633-17.968-30.34-20.853-12.233-2.776-24.047 1.462-35.472 5.563-7.769 2.787-15.108 5.421-21.488 5.421s-13.719-2.634-21.488-5.422c-11.424-4.1-23.237-8.339-35.472-5.563-12.707 2.884-22.075 12.43-30.34 20.853-5.597 5.703-10.883 11.09-16.255 13.678-5.562 2.679-13.201 3.479-21.29 4.327-11.623 1.218-24.796 2.598-34.774 10.554-10.031 7.998-14.306 20.554-18.077 31.632-2.606 7.656-5.068 14.887-8.88 19.654-3.772 4.718-10.258 8.694-17.124 12.904-10.019 6.144-21.375 13.107-26.99 24.728-5.518 11.421-4.001 23.952-2.534 36.071.981 8.102 1.908 15.754.522 21.841-1.299 5.707-5.348 12.001-9.635 18.665-6.404 9.956-13.663 21.24-13.663 34.378s7.259 24.422 13.663 34.377c4.287 6.665 8.336 12.959 9.635 18.666 1.386 6.087.459 13.739-.522 21.841-1.467 12.119-2.985 24.65 2.533 36.07 5.615 11.621 16.971 18.585 26.991 24.728 6.866 4.21 13.351 8.186 17.123 12.904 3.812 4.767 6.274 11.998 8.88 19.653 3.771 11.079 8.046 23.635 18.077 31.633 9.979 7.956 23.152 9.336 34.774 10.554 8.088.847 15.728 1.647 21.29 4.327 5.372 2.588 10.659 7.975 16.256 13.678 8.265 8.422 17.633 17.968 30.34 20.853 12.236 2.777 24.048-1.461 35.472-5.563 7.77-2.788 15.108-5.422 21.488-5.422s13.719 2.634 21.488 5.422c8.874 3.185 17.98 6.453 27.338 6.453 2.69 0 5.402-.27 8.134-.89 12.707-2.884 22.075-12.43 30.34-20.852 5.597-5.704 10.883-11.09 16.255-13.678 5.562-2.68 13.202-3.48 21.29-4.327 11.623-1.218 24.795-2.598 34.774-10.554 10.031-7.998 14.305-20.554 18.077-31.632 2.606-7.656 5.068-14.887 8.88-19.654 3.772-4.718 10.258-8.694 17.124-12.904 10.019-6.144 21.375-13.107 26.99-24.728 5.518-11.421 4.001-23.952 2.534-36.071-.981-8.102-1.908-15.754-.522-21.841 1.298-5.707 5.348-12.001 9.635-18.665zm-16.82-10.821c-5.033 7.824-10.237 15.914-12.316 25.046-2.158 9.478-.976 19.243.168 28.685 1.172 9.682 2.279 18.827-.687 24.966-3.063 6.338-11.016 11.215-19.437 16.378-8.029 4.923-16.332 10.014-22.289 17.463-5.998 7.5-9.147 16.751-12.193 25.699-3.167 9.304-6.16 18.092-11.613 22.44-5.407 4.312-14.628 5.277-24.389 6.3-9.434.988-19.189 2.01-27.886 6.2-8.52 4.104-15.297 11.01-21.851 17.688-6.951 7.083-13.517 13.773-20.492 15.357-6.544 1.483-15.164-1.608-24.29-4.883-9.038-3.244-18.384-6.598-28.244-6.598s-19.206 3.354-28.244 6.598c-9.126 3.275-17.746 6.368-24.29 4.883-6.975-1.584-13.541-8.274-20.492-15.357-6.554-6.678-13.331-13.584-21.851-17.688-8.697-4.19-18.452-5.211-27.886-6.2-9.762-1.022-18.982-1.989-24.389-6.3-5.453-4.348-8.445-13.136-11.612-22.44-3.046-8.947-6.196-18.199-12.193-25.699-5.957-7.449-14.259-12.539-22.289-17.463-8.421-5.163-16.375-10.039-19.437-16.378-2.966-6.139-1.859-15.284-.687-24.966 1.144-9.442 2.326-19.206.168-28.685-2.079-9.132-7.283-17.223-12.316-25.047-5.388-8.379-10.48-16.296-10.48-23.556 0-7.261 5.092-15.177 10.483-23.558 5.033-7.824 10.237-15.914 12.316-25.046 2.158-9.479.976-19.243-.168-28.685-1.172-9.682-2.279-18.827.687-24.966 3.063-6.338 11.016-11.215 19.437-16.378 8.029-4.923 16.332-10.014 22.289-17.463 5.997-7.499 9.147-16.752 12.193-25.699 3.168-9.304 6.16-18.092 11.613-22.44 5.408-4.312 14.628-5.277 24.39-6.3 9.434-.988 19.188-2.01 27.886-6.199 8.52-4.104 15.296-11.01 21.85-17.688 6.951-7.083 13.517-13.773 20.492-15.357 6.542-1.484 15.164 1.608 24.29 4.883 9.036 3.242 18.382 6.596 28.242 6.596s19.206-3.354 28.244-6.598c9.126-3.274 17.744-6.367 24.29-4.883 6.975 1.583 13.541 8.274 20.492 15.357 6.554 6.678 13.331 13.584 21.851 17.688 8.697 4.189 18.452 5.211 27.886 6.2 9.762 1.023 18.982 1.989 24.39 6.3 5.453 4.348 8.445 13.136 11.612 22.441 3.046 8.947 6.196 18.199 12.193 25.699 5.957 7.449 14.259 12.54 22.289 17.463 8.421 5.163 16.375 10.039 19.437 16.378 2.966 6.139 1.859 15.284.687 24.966-1.144 9.442-2.326 19.207-.168 28.685 2.079 9.132 7.283 17.222 12.316 25.046 5.389 8.381 10.481 16.297 10.481 23.558 0 7.26-5.092 15.177-10.483 23.557z">
                                            </path>
                                        </g>
                                        <g>
                                            <path
                                                d="m256 323.636-57.162 20.326 1.668-60.645-36.995-48.082 58.192-17.154 34.297-50.044 34.297 50.044 58.192 17.154-36.995 48.082 1.668 60.645z"
                                                fill="#e2c4ff"></path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> ضمانت اصل‌بودن کالا </span>
                                <span class="subtitle"> تایید اصالت کالا </span>
                            </div>
                        </div>
                    </div>
                </section>

                <section class="mb-4">
                    <div class="row">
                        <div class="d-flex">
                            <div class="col-lg-4 col-md-4">
                                <div class="p-10">
                                    <span class="title-14 title "> با نگارشاپ </span>
                                    <div class="list-item">
                                        <ul>
                                            <li>
                                                <router-link> اتاق خبر نگارشاپ </router-link>
                                            </li>
                                            <li>
                                                <router-link> فروش در نگارشاپ </router-link>
                                            </li>
                                            <li>
                                                <router-link>همکاری با سازمان‌ها </router-link>
                                            </li>
                                            <li>
                                                <router-link> فرصت‌های شغلی </router-link>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-4 col-md-4">
                                <div class="p-10">
                                    <span class="title-14 title "> خدمات مشتریان </span>
                                    <div class="list-item">
                                        <ul>
                                            <li>
                                                <router-link> پاسخ به پرسش‌های متداول </router-link>
                                            </li>
                                            <li>
                                                <router-link> رویه‌های بازگرداندن کالا </router-link>
                                            </li>
                                            <li>
                                                <router-link> شرایط استفاده </router-link>
                                            </li>
                                            <li>
                                                <router-link> حریم خصوصی </router-link>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-4 col-md-4">
                                <div class="p-10">
                                    <span class="title-14 title "> راهنمای خرید از نگارشاپ </span>
                                    <div class="list-item">
                                        <ul>
                                            <li>
                                                <router-link> نحوه ثبت سفارش </router-link>
                                            </li>
                                            <li>
                                                <router-link> رویه ارسال سفارش </router-link>
                                            </li>
                                            <li>
                                                <router-link> شیوه‌های پرداخت </router-link>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>

                <section>
                    <div class="p-10 mb-3">
                        <div class="d-flex flex-column">
                            <span class="title-14 title "> خبرنامه </span>
                            <span class="title-13 title "> با عضویت در خبرنامه از آخرین اخبار و محصولات سایت
                                مطلع شوید... </span>
                        </div>
                        <div class="input-form">
                            <input type="email" placeholder="آدرس ایمیل">
                            <button class="input-append" type="submit"> عضوم کن </button>
                        </div>
                    </div>
                </section>

                <section>
                    <p class="title-13 title ">تمامی مطالب، عکس ها و… مطعلق به سایت نگارشاپ می باشد. طراح: پسرک کدنویس
                    </p>
                </section>

                <section>
                    <ul class="fixed-bottom-bar-responsive">
                        <li>
                            <button class="bottom-bar" type="button" aria-label="مقایسه" data-toggle="tooltip"
                                data-placement="top" title="" data-original-title="مقایسه">
                                <font-awesome-icon icon="shuffle" />
                            </button>
                        </li>
                        <li>
                            <button class="bottom-bar" type="button">
                                <font-awesome-icon icon="heart" />
                            </button>
                        </li>
                        <li>
                            <button class="bottom-bar" type="button">
                                <font-awesome-icon icon="cart-shopping" />
                            </button>
                        </li>
                        <li>
                            <button class="bottom-bar" type="button">
                                <font-awesome-icon icon="angle-up" />
                            </button>
                        </li>
                    </ul>
                </section>
            </div>
        </footer>

        <!-- footer-sm -->
        <footer class="site-footer responsive-footer-mobile">
            <div class="shape-top">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none">
                    <path class="shape-fill" opacity="0.33"
                        d="M473,67.3c-203.9,88.3-263.1-34-320.3,0C66,119.1,0,59.7,0,59.7V0h1000v59.7 c0,0-62.1,26.1-94.9,29.3c-32.8,3.3-62.8-12.3-75.8-22.1C806,49.6,745.3,8.7,694.9,4.7S492.4,59,473,67.3z">
                    </path>
                    <path class="shape-fill" opacity="0.66"
                        d="M734,67.3c-45.5,0-77.2-23.2-129.1-39.1c-28.6-8.7-150.3-10.1-254,39.1 s-91.7-34.4-149.2,0C115.7,118.3,0,39.8,0,39.8V0h1000v36.5c0,0-28.2-18.5-92.1-18.5C810.2,18.1,775.7,67.3,734,67.3z">
                    </path>
                    <path class="shape-fill"
                        d="M766.1,28.9c-200-57.5-266,65.5-395.1,19.5C242,1.8,242,5.4,184.8,20.6C128,35.8,132.3,44.9,89.9,52.5C28.6,63.7,0,0,0,0 h1000c0,0-9.9,40.9-83.6,48.1S829.6,47,766.1,28.9z">
                    </path>
                </svg>
            </div>

            <div class="container">
                <section class="footer-item-box mb-5">
                    <div class="row flex-nowrap flex-lg-wrap mb-5">
                        <div class="col-6 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path
                                                d="m502 340.354h-.303v-58.216c-.004-1.814-.366-2.854-.723-3.74l-36.249-95.892c-1.47-3.89-5.194-6.464-9.354-6.464h-59.152v-62.676c0-15.024-12.215-27.246-27.229-27.246h-176.192c3.212 6.331 5.818 13.023 7.742 20h168.45c3.986 0 7.229 3.251 7.229 7.246v226.987h-285.919v-109.155c-6.909-1.099-13.597-2.86-20-5.217v114.373h-.303c-5.523 0-10 4.477-10 10v61.51c0 5.523 4.477 10 10 10h46.407c-.73 3.476-1.114 7.079-1.114 10.77 0 28.725 23.254 52.095 51.838 52.095 28.583 0 51.838-23.37 51.838-52.095 0-3.691-.384-7.294-1.114-10.77h147.24c-.73 3.476-1.114 7.079-1.114 10.77 0 28.725 23.255 52.095 51.839 52.095s51.838-23.37 51.838-52.095c0-3.691-.384-7.294-1.114-10.77h35.459c5.522 0 10-4.477 10-10v-61.51c0-5.522-4.478-10-10-10zm-105.779-144.311h52.24l28.766 76.095h-81.006zm0 96.095h85.477v48.211h-85.477zm-229.092 172.591c-17.556 0-31.838-14.397-31.838-32.095 0-17.697 14.283-32.095 31.838-32.095s31.838 14.397 31.838 32.095-14.282 32.095-31.838 32.095zm248.688 0c-17.556 0-31.839-14.397-31.839-32.095 0-17.697 14.283-32.095 31.839-32.095s31.838 14.397 31.838 32.095-14.282 32.095-31.838 32.095zm76.183-62.865h-34.24c-.045 0-.09.001-.135.002-9.442-12.923-24.666-21.326-41.807-21.326-17.14 0-32.364 8.403-41.806 21.325h-164.933c-.048 0-.095.001-.143.002-9.443-12.923-24.667-21.326-41.807-21.326s-32.363 8.403-41.806 21.325h-45.325v-41.51h412.002z"
                                                fill="#020288"></path>
                                        </g>
                                        <g>
                                            <ellipse cx="107.207" cy="134.9" fill="#e2c4ff" rx="107.207" ry="107.629">
                                            </ellipse>
                                            <path
                                                d="m125.859 149.581h-18.651c-5.523 0-10-4.477-10-10v-31.208c0-5.523 4.477-10 10-10s10 4.477 10 10v21.208h8.651c5.523 0 10 4.477 10 10 0 5.524-4.477 10-10 10z"
                                                fill="#020288"></path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> تحویل اکسپرس </span>
                                <span class="subtitle"> در کمترین زمان </span>
                            </div>
                        </div>

                        <div class="col-6 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path
                                                d="m292.28 2.536c-66.416 0-125.949 29.445-166.237 75.982 5.624 2.843 10.9 6.619 15.599 11.331l28.33 28.406c29.435-34.59 73.306-56.535 122.308-56.535 88.627 0 160.473 71.785 160.473 160.336 0 49.151-22.135 93.136-56.988 122.548l22.757 22.818c6.75 6.768 11.573 14.741 14.472 23.231 48.277-40.268 78.994-100.848 78.994-168.597-.001-121.237-98.367-219.52-219.708-219.52z"
                                                fill="#b2f0fb"></path>
                                        </g>
                                        <g fill="#020288">
                                            <path
                                                d="m425.602 360.361-32.672-32.76c-12.919-12.953-30.086-20.087-48.34-20.087s-35.422 7.134-48.342 20.089l-29.512 29.609-114.89-115.153 29.549-29.646c26.634-26.706 26.634-70.159 0-96.864l-32.672-32.76c-13.35-13.386-31.09-20.758-49.954-20.758s-36.604 7.372-49.954 20.758c-.274.275-.532.565-.772.87l-15.997 20.289c-15.27 19.367-25.587 42.398-29.837 66.604-4.286 24.413-2.311 49.581 5.71 72.784 5.812 16.812 13.122 29.71 21.726 38.338l197.771 198.251c6.821 6.84 16.396 12.92 28.46 18.072 18.002 7.688 37.12 11.456 56.211 11.455 31.786-.001 63.489-10.449 89.775-30.642l22.751-17.479c.348-.267.678-.558.988-.869 27.525-27.598 27.525-72.503.001-100.101zm-13.719 85.528-22.205 17.059c-36.35 27.926-84.611 34.307-125.948 16.655-9.696-4.141-17.149-8.785-22.154-13.803l-197.771-198.251c-6.452-6.47-12.167-16.815-16.983-30.749-14.134-40.886-6.31-85.921 20.93-120.47l15.62-19.811c9.523-9.347 22.072-14.488 35.397-14.488 13.511 0 26.222 5.285 35.792 14.881l32.672 32.76c18.867 18.918 18.867 49.7-.002 68.62l-36.591 36.711c-3.891 3.904-3.89 10.221.003 14.123l129.016 129.311c1.876 1.881 4.425 2.938 7.081 2.937 2.657-.001 5.205-1.059 7.08-2.94l36.589-36.709c9.139-9.164 21.277-14.21 34.179-14.21s25.04 5.046 34.179 14.21l32.672 32.76c19.61 19.661 19.758 51.559.444 71.404z">
                                            </path>
                                            <g>
                                                <path
                                                    d="m361.003 221.7h-1.121c.121-21.045.184-43.537-.076-46.664-.586-7.056-4.823-12.807-10.796-14.65-5.923-1.828-12.211.471-16.822 6.152-4.979 6.136-27.961 47.742-34.886 60.347-1.702 3.098-1.642 6.864.158 9.907 1.8 3.042 5.072 4.908 8.606 4.908h33.681c-.041 5.609-.084 10.986-.125 15.844-.047 5.523 4.391 10.038 9.914 10.085.029.001.059.001.088.001 5.482 0 9.951-4.421 9.997-9.914.026-2.999.074-8.807.128-16.016h1.253c5.523 0 10-4.477 10-10 .001-5.523-4.477-10-9.999-10zm-21.125 0h-16.853c6.251-11.227 12.345-21.998 16.965-29.943-.004 8.206-.048 18.878-.112 29.943z">
                                                </path>
                                                <path
                                                    d="m282.45 250.419c-8.794.111-18.124.184-25.861.199 4.857-6.523 11.614-15.793 20.75-28.902 5.593-8.025 9.24-15.609 10.84-22.541.375-1.584.688-4.6.689-5.647.018-17.91-14.528-32.48-32.385-32.48-15.444 0-28.809 11.02-31.779 26.204-1.06 5.42 2.474 10.673 7.895 11.733 5.419 1.059 10.673-2.475 11.733-7.895 1.138-5.819 6.248-10.042 12.151-10.042 6.649 0 12.09 5.307 12.373 11.938l-.272 2.136c-1.091 4.338-3.664 9.436-7.654 15.16-13.857 19.883-22.126 30.756-26.567 36.598-5.489 7.217-7.994 10.511-6.29 16.041 1.001 3.249 3.461 5.744 6.749 6.844 1.337.448 2.619.877 19.628.877 6.676 0 15.777-.066 28.251-.224 5.522-.069 9.943-4.603 9.873-10.125-.068-5.523-4.619-9.93-10.124-9.874z">
                                                </path>
                                            </g>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> پشتیبانی ۲۴ ساعته </span>
                                <span class="subtitle"> پشتیبانی حرفه ای </span>
                            </div>
                        </div>
                    </div>

                    <div class="row flex-nowrap flex-lg-wrap mb-5">
                        <div class="col-6 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path d="m182.8 245.634h146.401v175.733h-146.401z" fill="#b2f0fb">
                                            </path>
                                        </g>
                                        <g fill="#020288">
                                            <path
                                                d="m249.334 472.084h13.333c5.523 0 10-4.478 10-10 0-5.523-4.477-10-10-10h-13.333c-5.523 0-10 4.477-10 10 0 5.522 4.477 10 10 10z">
                                            </path>
                                            <path
                                                d="m414.606 147.622v-99.52c0-26.523-21.661-48.102-48.285-48.102h-220.642c-26.624 0-48.284 21.579-48.284 48.103v99.516c-.007.429-.007.857 0 1.284v314.904c0 26.574 21.619 48.193 48.193 48.193h220.825c26.574 0 48.193-21.619 48.193-48.193v-314.908c.006-.424.007-.85 0-1.277zm-20 1.097c-.111 4.863-2.034 9.425-5.466 12.922-3.701 3.773-8.675 5.851-14.005 5.851-10.909 0-19.784-8.875-19.784-19.784 0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784s-19.784-8.875-19.784-19.784c0-5.522-4.477-10-10-10s-10 4.478-10 10c0 10.909-8.875 19.784-19.784 19.784-5.329 0-10.303-2.077-14.004-5.85-3.431-3.497-5.354-8.058-5.466-12.919v-64.178h277.211v64.174zm-277.211 33.701c5.863 3.311 12.52 5.071 19.47 5.071 11.84 0 22.49-5.2 29.784-13.436 7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436s22.49-5.2 29.784-13.436c7.294 8.235 17.944 13.436 29.784 13.436 6.95 0 13.607-1.761 19.471-5.071v229.748h-277.213zm28.284-162.42h220.643c15.596 0 28.284 12.606 28.284 28.103v16.442h-277.211v-16.442c0-15.497 12.688-28.103 28.284-28.103zm248.927 443.807c0 15.546-12.648 28.193-28.193 28.193h-220.826c-15.545 0-28.193-12.647-28.193-28.193v-31.639h277.211v31.639z">
                                            </path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title">پرداخت در محل </span>
                                <span class="subtitle"> پرداخت هنگام دریافت </span>
                            </div>
                        </div>

                        <div class="col-6 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g>
                                            <path
                                                d="m469.871 159.668c1.879-1.876 2.935-4.422 2.935-7.077s-1.056-5.201-2.935-7.077l-53.256-53.161c-3.904-3.896-10.226-3.896-14.13 0l-27.381 27.332c-24.474-16.378-51.956-27.45-81.005-32.619 8.582-9.254 13.835-21.629 13.835-35.207 0-28.595-23.297-51.859-51.934-51.859s-51.934 23.264-51.934 51.859c0 13.577 5.252 25.953 13.835 35.207-29.049 5.169-56.531 16.241-81.005 32.619l-27.381-27.332c-3.904-3.896-10.226-3.896-14.13 0l-53.256 53.161c-1.879 1.876-2.935 4.422-2.935 7.077s1.056 5.201 2.935 7.077l29.173 29.122c-19.437 32.673-29.807 70.076-29.807 109.069 0 57.202 22.314 110.98 62.833 151.427 40.514 40.442 94.379 62.714 151.672 62.714s111.158-22.272 151.671-62.714c40.519-40.446 62.833-94.225 62.833-151.427 0-38.994-10.37-76.396-29.807-109.07zm-60.321-46.108 39.101 39.031-19.215 19.181c-6.53-8.912-13.795-17.383-21.765-25.339-5.221-5.211-10.664-10.121-16.307-14.72zm-153.55-93.56c17.608 0 31.934 14.292 31.934 31.859 0 14.084-9.209 26.063-21.934 30.259v-23.11c0-5.522-4.477-10-10-10s-10 4.478-10 10v23.11c-12.726-4.196-21.934-16.175-21.934-30.259 0-17.567 14.326-31.859 31.934-31.859zm-192.651 132.591 39.101-39.031 18.186 18.153c-5.643 4.599-11.086 9.508-16.307 14.72-7.97 7.956-15.235 16.428-21.765 25.339zm192.651 339.409c-107.251 0-194.505-87.091-194.505-194.141s87.254-194.14 194.505-194.14 194.505 87.091 194.505 194.141-87.255 194.14-194.505 194.14z"
                                                fill="#020288"></path>
                                        </g>
                                        <g>
                                            <g>
                                                <ellipse cx="256" cy="297.859" fill="#e2c4ff" rx="152.994" ry="152.721">
                                                </ellipse>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m256 195.824c-5.523 0-10-4.478-10-10v-3.411c0-5.523 4.477-10 10-10s10 4.477 10 10v3.411c0 5.523-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m256 423.306c-5.523 0-10-4.478-10-10v-3.411c0-5.522 4.477-10 10-10s10 4.478 10 10v3.411c0 5.522-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m371.446 307.859h-3.412c-5.523 0-10-4.478-10-10s4.477-10 10-10h3.412c5.523 0 10 4.478 10 10 0 5.523-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m143.965 307.859h-3.412c-5.523 0-10-4.478-10-10s4.477-10 10-10h3.412c5.523 0 10 4.478 10 10 0 5.523-4.477 10-10 10z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m335.221 228.639c-2.559 0-5.119-.977-7.071-2.929-3.905-3.905-3.905-10.237 0-14.143l2.412-2.412c3.905-3.904 10.237-3.904 14.143 0 3.905 3.905 3.905 10.237 0 14.143l-2.412 2.412c-1.954 1.952-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m174.367 389.492c-2.559 0-5.119-.977-7.071-2.929-3.905-3.905-3.905-10.237 0-14.143l2.412-2.412c3.905-3.904 10.237-3.904 14.143 0 3.905 3.905 3.905 10.237 0 14.143l-2.412 2.412c-1.953 1.953-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <g>
                                                <g>
                                                    <path
                                                        d="m337.633 389.492c-2.559 0-5.119-.977-7.071-2.929l-2.412-2.412c-3.905-3.905-3.905-10.237 0-14.143 3.905-3.904 10.237-3.904 14.143 0l2.412 2.412c3.905 3.905 3.905 10.237 0 14.143-1.954 1.953-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                                <g>
                                                    <path
                                                        d="m176.779 228.639c-2.559 0-5.119-.977-7.071-2.929l-2.412-2.412c-3.905-3.905-3.905-10.237 0-14.143 3.905-3.904 10.237-3.904 14.143 0l2.412 2.412c3.905 3.905 3.905 10.237 0 14.143-1.953 1.952-4.513 2.929-7.072 2.929z"
                                                        fill="#020288"></path>
                                                </g>
                                            </g>
                                            <path
                                                d="m315.809 286.361h-25.278c-3.337-11.001-12.031-19.695-23.033-23.032v-31.308c0-5.523-4.477-10-10-10s-10 4.477-10 10v31.308c-14.168 4.297-24.511 17.48-24.511 33.032 0 19.029 15.482 34.511 34.511 34.511 15.553 0 28.736-10.342 33.033-24.511h25.278c5.523 0 10-4.478 10-10s-4.477-10-10-10zm-58.311 24.511c-8.001 0-14.511-6.51-14.511-14.511s6.509-14.51 14.511-14.51c8.001 0 14.511 6.509 14.511 14.51-.001 8.001-6.51 14.511-14.511 14.511z"
                                                fill="#020288"></path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> ۷ روز ضمانت </span>
                                <span class="subtitle"> مهلت بازگشت وجه </span>
                            </div>
                        </div>
                    </div>

                    <div class="row flex-nowrap flex-lg-wrap">
                        <div class="col-6 d-flex flex-row">
                            <div class="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" height="40" viewBox="0 0 512 512"
                                    width="40">
                                    <g>
                                        <g fill="#020288">
                                            <path
                                                d="m256 82.637c-95.593 0-173.363 77.77-173.363 173.363s77.77 173.363 173.363 173.363 173.363-77.77 173.363-173.363-77.77-173.363-173.363-173.363zm0 326.726c-84.565 0-153.363-68.798-153.363-153.363s68.798-153.363 153.363-153.363 153.363 68.798 153.363 153.363-68.799 153.363-153.363 153.363z">
                                            </path>
                                            <path
                                                d="m498.337 290.378c6.404-9.956 13.663-21.24 13.663-34.378s-7.259-24.422-13.663-34.378c-4.287-6.664-8.336-12.958-9.635-18.665-1.386-6.087-.459-13.74.522-21.841 1.467-12.119 2.985-24.65-2.533-36.07-5.615-11.621-16.971-18.584-26.991-24.728-6.866-4.209-13.351-8.186-17.123-12.904-3.812-4.767-6.274-11.998-8.88-19.653-3.772-11.079-8.046-23.635-18.077-31.633-9.979-7.956-23.151-9.336-34.774-10.554-8.088-.847-15.728-1.647-21.29-4.327-5.372-2.588-10.659-7.975-16.256-13.678-8.265-8.422-17.633-17.968-30.34-20.853-12.233-2.776-24.047 1.462-35.472 5.563-7.769 2.787-15.108 5.421-21.488 5.421s-13.719-2.634-21.488-5.422c-11.424-4.1-23.237-8.339-35.472-5.563-12.707 2.884-22.075 12.43-30.34 20.853-5.597 5.703-10.883 11.09-16.255 13.678-5.562 2.679-13.201 3.479-21.29 4.327-11.623 1.218-24.796 2.598-34.774 10.554-10.031 7.998-14.306 20.554-18.077 31.632-2.606 7.656-5.068 14.887-8.88 19.654-3.772 4.718-10.258 8.694-17.124 12.904-10.019 6.144-21.375 13.107-26.99 24.728-5.518 11.421-4.001 23.952-2.534 36.071.981 8.102 1.908 15.754.522 21.841-1.299 5.707-5.348 12.001-9.635 18.665-6.404 9.956-13.663 21.24-13.663 34.378s7.259 24.422 13.663 34.377c4.287 6.665 8.336 12.959 9.635 18.666 1.386 6.087.459 13.739-.522 21.841-1.467 12.119-2.985 24.65 2.533 36.07 5.615 11.621 16.971 18.585 26.991 24.728 6.866 4.21 13.351 8.186 17.123 12.904 3.812 4.767 6.274 11.998 8.88 19.653 3.771 11.079 8.046 23.635 18.077 31.633 9.979 7.956 23.152 9.336 34.774 10.554 8.088.847 15.728 1.647 21.29 4.327 5.372 2.588 10.659 7.975 16.256 13.678 8.265 8.422 17.633 17.968 30.34 20.853 12.236 2.777 24.048-1.461 35.472-5.563 7.77-2.788 15.108-5.422 21.488-5.422s13.719 2.634 21.488 5.422c8.874 3.185 17.98 6.453 27.338 6.453 2.69 0 5.402-.27 8.134-.89 12.707-2.884 22.075-12.43 30.34-20.852 5.597-5.704 10.883-11.09 16.255-13.678 5.562-2.68 13.202-3.48 21.29-4.327 11.623-1.218 24.795-2.598 34.774-10.554 10.031-7.998 14.305-20.554 18.077-31.632 2.606-7.656 5.068-14.887 8.88-19.654 3.772-4.718 10.258-8.694 17.124-12.904 10.019-6.144 21.375-13.107 26.99-24.728 5.518-11.421 4.001-23.952 2.534-36.071-.981-8.102-1.908-15.754-.522-21.841 1.298-5.707 5.348-12.001 9.635-18.665zm-16.82-10.821c-5.033 7.824-10.237 15.914-12.316 25.046-2.158 9.478-.976 19.243.168 28.685 1.172 9.682 2.279 18.827-.687 24.966-3.063 6.338-11.016 11.215-19.437 16.378-8.029 4.923-16.332 10.014-22.289 17.463-5.998 7.5-9.147 16.751-12.193 25.699-3.167 9.304-6.16 18.092-11.613 22.44-5.407 4.312-14.628 5.277-24.389 6.3-9.434.988-19.189 2.01-27.886 6.2-8.52 4.104-15.297 11.01-21.851 17.688-6.951 7.083-13.517 13.773-20.492 15.357-6.544 1.483-15.164-1.608-24.29-4.883-9.038-3.244-18.384-6.598-28.244-6.598s-19.206 3.354-28.244 6.598c-9.126 3.275-17.746 6.368-24.29 4.883-6.975-1.584-13.541-8.274-20.492-15.357-6.554-6.678-13.331-13.584-21.851-17.688-8.697-4.19-18.452-5.211-27.886-6.2-9.762-1.022-18.982-1.989-24.389-6.3-5.453-4.348-8.445-13.136-11.612-22.44-3.046-8.947-6.196-18.199-12.193-25.699-5.957-7.449-14.259-12.539-22.289-17.463-8.421-5.163-16.375-10.039-19.437-16.378-2.966-6.139-1.859-15.284-.687-24.966 1.144-9.442 2.326-19.206.168-28.685-2.079-9.132-7.283-17.223-12.316-25.047-5.388-8.379-10.48-16.296-10.48-23.556 0-7.261 5.092-15.177 10.483-23.558 5.033-7.824 10.237-15.914 12.316-25.046 2.158-9.479.976-19.243-.168-28.685-1.172-9.682-2.279-18.827.687-24.966 3.063-6.338 11.016-11.215 19.437-16.378 8.029-4.923 16.332-10.014 22.289-17.463 5.997-7.499 9.147-16.752 12.193-25.699 3.168-9.304 6.16-18.092 11.613-22.44 5.408-4.312 14.628-5.277 24.39-6.3 9.434-.988 19.188-2.01 27.886-6.199 8.52-4.104 15.296-11.01 21.85-17.688 6.951-7.083 13.517-13.773 20.492-15.357 6.542-1.484 15.164 1.608 24.29 4.883 9.036 3.242 18.382 6.596 28.242 6.596s19.206-3.354 28.244-6.598c9.126-3.274 17.744-6.367 24.29-4.883 6.975 1.583 13.541 8.274 20.492 15.357 6.554 6.678 13.331 13.584 21.851 17.688 8.697 4.189 18.452 5.211 27.886 6.2 9.762 1.023 18.982 1.989 24.39 6.3 5.453 4.348 8.445 13.136 11.612 22.441 3.046 8.947 6.196 18.199 12.193 25.699 5.957 7.449 14.259 12.54 22.289 17.463 8.421 5.163 16.375 10.039 19.437 16.378 2.966 6.139 1.859 15.284.687 24.966-1.144 9.442-2.326 19.207-.168 28.685 2.079 9.132 7.283 17.222 12.316 25.046 5.389 8.381 10.481 16.297 10.481 23.558 0 7.26-5.092 15.177-10.483 23.557z">
                                            </path>
                                        </g>
                                        <g>
                                            <path
                                                d="m256 323.636-57.162 20.326 1.668-60.645-36.995-48.082 58.192-17.154 34.297-50.044 34.297 50.044 58.192 17.154-36.995 48.082 1.668 60.645z"
                                                fill="#e2c4ff"></path>
                                        </g>
                                    </g>
                                </svg>
                            </div>
                            <div class="content d-flex flex-column">
                                <span class="title-13 title"> ضمانت اصل‌بودن کالا </span>
                                <span class="subtitle"> تایید اصالت کالا </span>
                            </div>
                        </div>
                    </div>
                </section>

                <section class="mb-4">
                    <div class="row">
                        <div class="d-flex">
                            <div class="col-6 col-md-4">
                                <div class="p-10">
                                    <span class="title-14 title "> با نگارشاپ </span>
                                    <div class="list-item">
                                        <ul>
                                            <li>
                                                <router-link> اتاق خبر نگارشاپ </router-link>
                                            </li>
                                            <li>
                                                <router-link> فروش در نگارشاپ </router-link>
                                            </li>
                                            <li>
                                                <router-link>همکاری با سازمان‌ها </router-link>
                                            </li>
                                            <li>
                                                <router-link> فرصت‌های شغلی </router-link>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>

                            <div class="col-6 col-md-4">
                                <div class="p-10">
                                    <span class="title-14 title "> خدمات مشتریان </span>
                                    <div class="list-item">
                                        <ul>
                                            <li>
                                                <router-link> پاسخ به پرسش‌های متداول </router-link>
                                            </li>
                                            <li>
                                                <router-link> رویه‌های بازگرداندن کالا </router-link>
                                            </li>
                                            <li>
                                                <router-link> شرایط استفاده </router-link>
                                            </li>
                                            <li>
                                                <router-link> حریم خصوصی </router-link>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="row">
                        <div class="col-6 col-md-4">
                            <div class="p-10">
                                <span class="title-14 title "> راهنمای خرید از نگارشاپ </span>
                                <div class="list-item">
                                    <ul>
                                        <li>
                                            <router-link> نحوه ثبت سفارش </router-link>
                                        </li>
                                        <li>
                                            <router-link> رویه ارسال سفارش </router-link>
                                        </li>
                                        <li>
                                            <router-link> شیوه‌های پرداخت </router-link>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </section>

                <section>
                    <div class="p-10 mb-3">
                        <div class="d-flex flex-column">
                            <span class="title-14 title "> خبرنامه </span>
                            <span class="title-13 title "> با عضویت در خبرنامه از آخرین اخبار و محصولات سایت
                                مطلع شوید... </span>
                        </div>
                        <div class="input-form">
                            <input type="email" placeholder="آدرس ایمیل">
                            <button class="input-append" type="submit"> عضوم کن </button>
                        </div>
                    </div>
                </section>

                <section>
                    <p class="title-13 title ">تمامی مطالب، عکس ها و… مطعلق به سایت نگارشاپ می باشد. طراح: پسرک کدنویس
                    </p>
                </section>
            </div>
        </footer>

    </div>
</template>


<script>



export default {
    name: 'Footer',

    data() {
        return {
            symbolOfTrusts: [
                {
                    thumbnail: 'https://demo.coderboy.ir/negarshop/wp-content/uploads/2020/07/logo.aspx_.png',
                    alt: 'logo-1'
                },
                {
                    thumbnail: 'https://demo.coderboy.ir/negarshop/wp-content/uploads/2020/07/50a7c6fb.png',
                    alt: 'logo-2'
                }
            ]
        }
    }
}
</script>


<style scoped lang="scss">
.container {
    @media (max-width:820px) {
        min-width: 98%;
    }
}

.p-10 {
    padding: 10px;
}

.site-footer {
    position: relative;
    background-color: #fff;
    margin-top: 30px;
    margin-bottom: 0px;
    padding: 120px 0px 0px 0px;
}

.footer-item-box {
    margin-bottom: 35px;
}

.shape-top {
    overflow: hidden;
    position: absolute;
    left: 0;
    width: 100%;
    line-height: 0;
    direction: ltr;
    top: -1px;
}

.shape-top svg {
    width: calc(200% + 1.3px);
    height: 100px;
    display: block;
    position: relative;
    left: 50%;
    transform: translateX(-50%);
    overflow: hidden;
}

.shape-fill {
    fill: #EEEEEE;
    transform-origin: center;
    transform: rotateY(0deg);
}

.title {
    color: #666666;
    font-weight: 500;
    margin-bottom: 5px;

    &-13 {
        font-size: 13px;
    }

    &-14 {
        font-size: 14px;
    }
}

.subtitle {
    color: #757575;
    font-size: 12px;
    font-weight: 400;
}

.desk {
    font-size: 13px;
    color: #757575;
    line-height: 1.75;
    text-align: justify;
}

.content {
    margin-right: 10px;
}

.list-item {
    margin-top: 10px;
}

ul {
    list-style-type: none;
    display: flex;
    flex-direction: column;
    padding: 0;
    margin: 0;

    li {
        position: relative;
        cursor: pointer;
        margin-left: 15px;
        transition: .2S;
        line-height: 1.75;

        a {
            text-decoration: none;
            color: #757575;
            display: block;
            font-weight: 400;
            font-size: 14px;
            transition: .2S;

            &:is(:hover, :focus) {
                color: #1b1b1b;
            }
        }
    }
}

.input-form {
    margin-top: 10px;
}

.input-form input {
    background: #f5f5f5;
    border: none;
    transition: 0.2s;
    font-size: 13px;
    border-radius: 15px;
    padding: 10px;
    color: #495057;
    height: auto;
    outline: none;
}

.input:focus {
    background-color: #fff;
    box-shadow: 2px 4px 4px #dedede;
}

.input-append {
    margin-right: 10px;
    border-radius: 15px;
    font-weight: 500;
    text-align: center;
    padding: 10px 10px;
    font-size: 13px;
    line-height: 1.25;
    border: none;
    background-color: #757575;
    color: #fff;
    transition: .3s;

    &:is(:hover, :focus) {
        background-color: #afaeae;
    }
}

.logo-img {
    width: 90%;
    height: auto;
}

.fixed-bottom-bar {
    position: fixed;
    bottom: 20px;
    margin: 0 -70px 0 0;
    padding: 0;
    list-style: none;
    white-space: nowrap;
    z-index: 999;
}

.bottom-bar {
    width: 50px;
    height: 50px;
    text-align: center;
    line-height: 0;
    font-size: 18px;
    display: block;
    padding: 0;
    background: #fff;
    color: #757575;
    box-shadow: rgba(0, 0, 0, 0.2) 0 2px 3px;
    border-radius: 20px;
    border: none;
    outline: none;
    margin-bottom: 5px;
    transition: .2s;

    &:is(:hover, :focus) {
        color: #495057;
    }
}

//responsive
.responsive-footer-lg {
    display: block;

    @media (max-width:820px) {
        display: none;
    }
}

.responsive-footer-tablet {
    display: none;

    @media (min-width:450px) and (max-width:820px) {
        display: block;
    }
}

.responsive-footer-mobile {
    display: none;

    @media (max-width:449px) {
        display: block;
    }
}

.fixed-bottom-bar-responsive {
    position: fixed;
    bottom: 20px;
    padding: 0;
    list-style: none;
    white-space: nowrap;
    z-index: 999;
}
</style>